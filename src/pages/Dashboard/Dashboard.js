...
// Telegram Bot token for SOS alerts
...
const Dashboard = () => {
  ...
  const [qrCode, setQrCode] = useState(""); // eslint-disable-line no-unused-vars
  ...
  const [newContactTelegram, setNewContactTelegram] = useState("");
  ...
  // Auto-update QR code URL in state when key fields change (no Firestore write)
  useEffect(() => {
    if (!user) return;
    const publicProfileUrl = `${window.location.origin}/public-profile?uid=${user.uid}&name=${encodeURIComponent(name || user.displayName || "")}`;
    const newQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(publicProfileUrl)}`;
  ...
  const handleSaveContact = async (e) => {
    ...
    if (emergencyContacts.length < 3 || editingContactIndex !== null) {
      const contactData = {
        ...
        telegramChatId: newContactTelegram.trim()
      };
      let updatedContacts;
      if (editingContactIndex === null) {
        updatedContacts = [...emergencyContacts, contactData];
      } else {
        updatedContacts = emergencyContacts.map((contact, index) =>
          index === editingContactIndex ? contactData : contact
        );
      }
  ...
  const handleEditExistingContact = (index) => {
    ...
    setNewContactTelegram(contact.telegramChatId || "");
  ...
  // Card 3: SOS — sends alert to all emergency contacts that have a Telegram Chat ID
  ...
  const sendTestSosNotification = async () => {
    ...
    const contactsWithTelegram = emergencyContacts.filter(c => c.telegramChatId);
    if (contactsWithTelegram.length === 0) {
      alert("No emergency contacts have a Telegram Chat ID. Please add one when creating a contact.");
      return;
    }
    ...
    // Build a rich SOS message using the user's medical profile
    const allergyText = allergies.length > 0 ? allergies.join(", ") : "None";
    const medsText = currentMeds.length > 0 ? currentMeds.join(", ") : "None";
    const bloodText = bloodGroup || "Unknown";
    const sosMessage = `🚨 SOS ALERT 🚨\n\nName: ${name || "Unknown"}\nBlood Group: ${bloodText}\nAllergies: ${allergyText}\nCurrent Medications: ${medsText}\nMobile: ${mobileNumber || "N/A"}\n\nThis person needs immediate help!`;

    const results = await Promise.allSettled(
      contactsWithTelegram.map(contact =>
        fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_id: contact.telegramChatId, text: sosMessage })
        }).then(r => r.json())
      )
    );
    ...
    setSosSending(false);
    setSosStatus(allOk ? "✅ SOS sent to all contacts!" : "⚠️ Some messages failed to send.");
  };
  ...
  const handleEditDetailsSubmit = async (e) => {
    ...
    const newMobile = e.target.mobileNumber.value;
      ...
      <div className="container">
        ...
        <div className="card" id="card1">
          ...
          {showAddContactForm && (
            <div className="contact-form">
              ...
              <input
                type="text"
                placeholder="Telegram Chat ID (for SOS alerts)"
        ...
        <div className="card" id="card3">
          ...
          <div className="sos-section">
            <button id="testSosBtn" onClick={sendTestSosNotification} disabled={sosSending}>
              {sosSending ? "Sending..." : "SOS"}
            </button>
            {sosStatus && (
              <p style={{ marginTop: "0.8rem", fontSize: "0.9rem", color: "#fff", textAlign: "center" }}>
                {sosStatus}
              </p>
            )}
            <p style={{ marginTop: "0.8rem", fontSize: "0.75rem", color: "rgba(255,255,255,0.8)", textAlign: "center" }}>
              Sends alert to all contacts with a Telegram Chat ID
            </p>
          </div>
        </div>
        ...
        <div className="card" id="card4">
          ...
          <form id="editForm" onSubmit={handleEditDetailsSubmit}>
            ...
            <input type="tel" id="mobileNumber" placeholder="Enter mobile number" defaultValue={mobileNumber} />
